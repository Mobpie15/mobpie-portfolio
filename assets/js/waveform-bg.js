...
(function() {
  ...
  function renderWaveform() {
    ...
    // Wave baseline: Positioned gracefully behind seated avatar (approx 40% from top)
    ...
    const alpha1 = audio.isPlaying ? (0.45 + smoothedBass * 0.45) : 0.32;
    ctx.strokeStyle = `rgba(255, 255, 255, ${alpha1.toFixed(3)})`;
    ctx.shadowColor = audio.isPlaying ? 'rgba(255, 255, 255, 0.9)' : 'rgba(255, 255, 255, 0.4)';
    ctx.shadowBlur = audio.isPlaying ? (10 + smoothedBass * 25) : 8;

    const baseAmp1 = audio.isPlaying ? (35 + smoothedBass * 85) : 22;

    for (let x = 0; x <= width; x += 6) {
      const normX = x / width;
      // Envelope to keep wave majestic in center and subtle at edges
    ...
    // -------------------------------------------------------------
    // WAVE 2: Secondary Mid-frequency Harmonic Titanium Wave
    // -------------------------------------------------------------
    ctx.beginPath();
    ctx.lineWidth = audio.isPlaying ? (1.2 + smoothedMid * 1.4) : 1.0;
    const alpha2 = audio.isPlaying ? (0.35 + smoothedMid * 0.35) : 0.22;
    ctx.strokeStyle = `rgba(196, 181, 253, ${alpha2.toFixed(3)})`;
    ...
    ctx.beginPath();
    ctx.lineWidth = 0.9;
    const alpha3 = audio.isPlaying ? (0.2 + smoothedEnergy * 0.25) : 0.14;
    ctx.strokeStyle = `rgba(161, 161, 170, ${alpha3.toFixed(3)})`;
    ctx.shadowBlur = 0;
